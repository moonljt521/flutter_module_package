#!/bin/bash
# This is flutter build  修改版 ： 适用于1.2.13 hotfix9 stable
...
# 1.12.13 额外需要
# step 3.3 在.android build.gradle中添加 maven { url "http://maven.17zuoye.net/nexus/content/groups/public/" }
if [ `grep -c 'maven.17zuoye.net/nexus/content/groups/public' .android/build.gradle` -eq '1' ]; then
            echo "maven { url 'http://maven.17zuoye.net/nexus/content/groups/public/' } 已存在 ！！！"
        else
            echo "maven { url 'http://maven.17zuoye.net/nexus/content/groups/public/' } 不存在，去添加"
            sed -i '' '17 a\
            maven { url "http://maven.17zuoye.net/nexus/content/groups/public/" }
            ' .android/build.gradle
        fi
...
echo '<<<<<<<<<<<<<<<<<<<<<<<<<< 结束 >>>>>>>>>>>>>>>>>>>>>>>>>'
echo '版本号：'$flutterOldAArVersion '--->' $flutterAArVersion
echo '打包成功 : yqx-flutter.aar.............最新版本为：'$flutterAArVersion
exit
